...
If by some odd chance a word has both front and back vowels, it is treated as either by random chance.
...
\begin{alignat*}{3}
  ...
  C_1\{\text{n}, \text{ŋ}\} C_2\{\text{k}, \text{ɡ}\} &\rightarrow \text{ɴ} C_2[+uv] &\quad [2 \mid \#\sigma] \\
  C_1[+av] &\rightarrow C_1[+rt] &\quad \left[\sum_{n \in \chi} n^2 \in \mathbb{P}\right] \\
\end{alignat*}

Some examples:

\begin{itemize}
    \item \hortho{swit} \emph{turning from turquoise to blue} /swit/ [s͎wit]
    \item \hortho{s\^hinke} \emph{does not go} /θxinkɛ/ [θxiɴqɛ] because the number of syllables is even
    \item \hortho{na.in16} \emph{but was waiting} /naʔin\{16\}/ [ɳaʔiɳ\{16\}] because $1^2 + 6^2 = 37$ is prime
\end{itemize}
...
\begin{table}[ht]
    ...
    \begin{tabular}{l|llllllllllll}
        ...
        \textnormal{∅} & &
        \Large\ltfont ~xa & \Large\ltfont ~xi & \Large\ltfont ~xy & \Large\ltfont ~xz &
        \Large\ltfont ~xe & \Large\ltfont ~xf & \Large\ltfont ~xm & \Large\ltfont ~xu &
        \Large\ltfont ~xw & \Large\ltfont ~xv & \Large\ltfont ~xo \\
        \vowr{a}{a}
        \vowr{i}{i}
        \vowr{i\^u}{y}
        \vowr{i\^o}{z}
...
~\\
\textkardinal{\hli{prin} \hlii{ji\^o} \hliii{hrihridek,} \hliv{senan} \hlv{ta} \hlvi{bes.}} \\
\hli{carry-\tsc{near}.\tsc{nonpast}} \hlii{\tsc{pr.other.sg}} \hliii{\tsc{def}\tl{book}-\tsc{sg},} \hliv{worry-\tsc{near}.\tsc{nonpast}} \hlv{\tsc{pr.near.int}} \hlvi{\tsc{pr.last\_clause}} \\
...
~\\
\textkardinal{\hli{prinha} \hlii{ji\^o} \hliii{hrihridek,} \hliv{senan} \hlv{ta} \hlvi{bes.}} \\
\hli{carry-\tsc{near}.\tsc{nonpast}-\tsc{interrogative}} \hlii{\tsc{pr.other.sg}} \hliii{\tsc{def}\tl{book}-\tsc{sg},} \hliv{worry-\tsc{near}.\tsc{nonpast}} \hlv{\tsc{pr.near.int}} \hlvi{\tsc{pr.last\_clause}} \\
...
\begin{longtabu} to \linewidth {|l|>{\kardinal}l|Y|}
    ...
    Epistemic necessity & ku\^im4 & An action that the speaker infers is happening. \\
...
\hli{and-\tsc{other.past}} \hlii{Ryse} \hliii{Tarul,} \hliv{and-\tsc{other.past}} \hlv{beef} \hlvi{soup,} \hlvii{eat-\tsc{other.past}} \hlviii{\tsc{def}\tl{}person-\tsc{du}} \hlix{\tsc{pr.anaph\_sub.cont}} \\
...
\hli{lose-\tsc{near.past}} \hlii{\tsc{pr.near.sg}} \hliii{\tsc{def}\tl{}book,} \hliv{give\_to-\tsc{anaph\_sub.past}} \hlv{Ryse-\tsc{acc}} \\
...
\begin{itemize}
    ...
    \item \textkardinal{t} $\leftrightarrow$ \textkardinal{k}, \textkardinal{p} $\leftrightarrow$ \textkardinal{t\^y} (in coda position)
    ...
    \item \textkardinal{r} $\leftrightarrow$ \textkardinal{c} (in any position other than after a fricative in an onset or in a coda)
    \item \textkardinal{j} $\leftrightarrow$ \textkardinal{l} (in any position other than in a coda)
    \item \textkardinal{r} $\leftrightarrow$ \textkardinal{l} (in coda position)
...
An example of usage: \\
~\\
\textkardinal{\hli{na.a} \hlii{banen-bus,} \hliii{[muta pre\^ome ri\^uk-\bs{xedri\^o}],} \hliv{s\^han} \hlv{mamasas} \hlvi{.im.}} \\
\hli{wait-\tsc{other}} \hlii{year-future,} \hliii{(name),} \hliv{go-\tsc{near}} \hlv{\tsc{def}\tl{}person-\tsc{sg}} \hlvi{\tsc{pr.anaph\_sub.sg}} \\
...
\begin{table}[H]
    \begin{adjustwidth}{-1cm}{}
        ...
        \begin{tabular}{
            ...
            YG & mo\^et & ru\^om & ho\^ep & di\^ul & n\^gi\^os & tek & n\^yar & f\^het & tse\^on\^y & yep & pu\^o \\
            Gn & t\^yo\^en & f\^xet\^y & jek & cum & go\^et & nu\^il & bon\^y & xe\^ot\^y & si\^ul & s\^hu\^os & ko \\
            Aq & s\^har & tur & ri\^um & wik & t\^yul & .u\^ik & txor & kros & nwep & tcap & be\^o \\
            Tu & fu\^il & n\^gu\^os & kek & d\^yi\^ur & myen & ris & hom & tyel & swit & du\^or & cu \\
            Cy & bi\^ur & do\^et & ni\^or & .it & cu\^is & bcek & txi\^ol & n\^yak & frin & bu\^it & no \\
            SB & pok & n\^yal & pe\^on\^y & txe\^ol & f\^xe\^om & txu\^or & wap & s\^hrop & pen & cip & tsi\^u \\
            Az & mar & so\^ek & xop & gce\^os & dwo\^er & n\^yat & fre\^ot\^y & fis & tir & rup & wa \\
            Bl & het\^y & tson\^y & f\^hur & nyo\^et\^y & f\^hyu\^ik & s\^hru\^in & bo\^en & du\^il & hak & ki\^un\^g & yu\^o \\
            Vi & do\^en\^g & wo\^et\^y & fi\^os & drat\^y & ti\^ol & pi\^ok & ru\^it\^y & ci\^ut\^y & gun\^g & hon & fa \\
            Gy & do & bi\^u & ke & pu & re & si & tsa & ya & f\^he & s\^hi & .is \\
        \end{tabular}
    \end{adjustwidth}
\end{table}
...
Oddly, transitions are idiomatic clauses whose literal meanings differ from (but are usually remotely related to) their actual meanings.

\begin{longtabu}[c]{>{\kardinal}YYY}
    \textnormal{Transition} & Literal & Actual \\
    \hline
    \endhead
    \endfoot
    fin te.er txarik & the rain joins the river & in addition, furthermore, moreover \\
    d\^yafin si\^ul-mifa s\^iul-narma & the child fox follows the parent fox & in the same way \\
    kcalan3 rumu\^i rilin & the tidal days are determined by the waves & of course, naturally \\
    potan nu\^iga mun\^go, make2 ron & the wolf chases the rabbit and eats it & one one hand \\
    mekin mun\^go, fagan6 nu\^inu\^iga memek & but another rabbit digs a hole and the wolf falls inside & on the other hand \\
    yunan2 \bs{}ci\^up suk & Řyp raises the sun & for instance, for example \\
    mesin te.er txoro & the rain tends the flowers & as a result, thus, therefore \\
    fin hacu\^o kakel, sin2 masa-napis-kika ra & the people of the present become statues and bones & finally, in conclusion, at last \\
    nu\^onat mexe \bs{}ci\^up & even Řyp was defeated by time & above all \\
    me\^odin txaris welse & the river or the forest & by all means, by no means (before a negative sentence) \\
\end{longtabu}
