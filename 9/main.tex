...
As with adjectives, relative clauses precede their antecedents when before the verb but follow them after the verb of the main clause. The relative clause is delimited from the antecedent by the particle \ortho{o}, and if the end of the embedded clause is not clear from context, the particle \ortho{ri} is placed on the other side of the clause.
...
~\\
\hli{Ramba} \hlii{furihta} \hliii{bana} \hliv{o} \hlv{ramba} \hlvi{haduþa} \hlvii{kadasu} \hlviii{nidu.} \\
\hli{man} \hlii{see-\tsc{inv}} \hliii{1\tsc{sg}} \hliv{\tsc{rel}} \hlv{man} \hlvi{go-\tsc{inf}} \hlvii{\tsc{past}} \hlviii{house} \\
\hlv{The man} \hli{whom} \hliii{I} \hlii{saw} \hlvi{went} \hlviii{home.}
...
\begin{itemize}
  \item \emph{Unmarked} (\tsc{unm}) is the default form of the noun.
  \item \emph{Reduced} (\tsc{red}) is closest to English's plural form and confers no additional properties to a group of objects. Compared with the unmarked N-E, the reduced N-E is used the most often with human nouns, less often with other animates and rarely with inanimate nouns. With uncountable nouns, the unmarked form is always used.
  \item \emph{Emergent} (\tsc{emg}) describes a group of objects with properties extending beyond its components but also individual qualities.
  \item \emph{Coherent} (\tsc{coh}) describes an entity that cannot be meaningfully divided into its individual parts.
\end{itemize}
...
\begin{itemize}
  \item \emph{Identical} (\tsc{id}) means that a similar group with $n$ elements as the one mentioned fall into $O(\log \log n)$ identities (to a margin of error) -- e.~g. a pile of candies or an orchard of apple trees.
  \item \emph{Similative} (\tsc{sim}) means that the members of the group in question are similar in name only -- e.~g. a forest with different species of plants.
  \item \emph{Related} (\tsc{rel}) means that at least one entity is the item in question and the others are related to it -- e.~g. spoons and other utensils.
\end{itemize}

\subsection{Specificity}

This refers to whether a noun phrase is unique in a given context and has two values: \emph{specific} (\tsc{spc}) and \emph{nonspecific} (\tsc{nsp}).
...
Adjectives are related to nouns; in fact, a noun can be made into an adjective without any marking. As said previously, an adjective will precede its antecedent if the antecedent occurs before the verb of the clause, and it will follow it otherwise. An adjective will modify its antecedent by exchanging its onsets with that of the antecedent: \ortho{\hliii{napu}} \emph{fish} and \ortho{\hliv{karaha}} \emph{fast} make \ortho{\hliv{k}\hliii{apu} \hliii{n}\hliv{araha}} \emph{a fast fish}.

The declension of an adjective agrees with that of the antecedent, and adjectives are declined in the same way as nouns.

An adjective can be made into an adverb by suffixing \ortho{-sþir} to the base form of the adjective.
...
\hli{Napuhta} \hlii{nigî} \hliii{ḟiruhta.} \\
\hli{fish-\tsc{unm.nsp}} \hlii{eat-\tsc{d.s.act}} \hliii{flower-\tsc{unm.nsp}} \\
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    \tikzset{
      angledArrow/.style={arrow,to path={(\tikztostart) -- ++(0pt,#1) \tikztonodes -| (\tikztotarget)}}
    }
    \node[stem] (v0) {0};
    \node[requiredAffix] (v1) [left=of v0] {1};
    \node[requiredAffix] (v2) [right=of v0] {2};
    \node[requiredAffix] (v3) [left=of v1] {3};
    \node[requiredAffix] (v4) [right=of v2] {4};
    
    % arrows
    \draw[arrow] (v0) to (v1);
    \draw[angledArrow=-1cm] (v1) to (v2);
    \draw[angledArrow=1cm] (v2) to (v3);
    \draw[angledArrow=-1.5cm] (v3) to (v4);
  \end{tikzpicture}
\end{figure}

All verbs that are in a position other than 0 are in the infinitive form, which involves suffixing \ortho{-þa}.
...
\subsection{A note about \ortho{imoþ}}

The dummy verb \ortho{imoþ} can disambiguate the verb of a sentence when it is not already clear.

\subsection{The copula}

The copula \ortho{oþ} is conjugated irregularly:

\begin{tablenf}
  \caption{Conjugations of \ortho{oþ}.}
  \centering
  \begin{tabular}{r|ll}
    Voice \bs{} Aspect & Simple & Prospective \\
    \hline
...
With \ortho{þdu} \emph{around}:

\begin{itemize}
  \item \hliii{napu} \emph{fish} → \hliii{na}\hliv{-þdu-}\hliii{pu} \emph{around the fish}
  \item \hliii{saginu} \emph{grove} → \hliii{sa}\hliv{-þdu-}\hliii{ginu} \emph{around the grove}
  \item \hliii{torþibam} \emph{crane (bird)} → \hliii{torþi}\hliv{-þdu-}\hliii{bam} \emph{around the crane}
  \item \hliii{þtarhaþpa} \emph{willow} → \hliii{þtarhaþ}\hliv{-þdu-}\hliii{pa} \emph{around the willow}
\end{itemize}
...
With \ortho{dira} \emph{to the right of:}

\begin{itemize}
  \item \hliii{napu} \emph{fish} → \hliii{na}\hliv{-dira-}\hliii{pu} \emph{to the right of to the fish}
  \item \hliii{saginu} \emph{grove} → \hliii{sa}\hliv{-dira-}\hliii{ginu} or \hliii{sagi}\hliv{-dira-}\hliii{nu} \emph{to the right of the grove}
  \item \hliii{torþibam} \emph{crane (bird)} → \hliii{tor}\hliv{-dira-}\hliii{þibam} or \hliii{torþi}\hliv{-dira-}\hliii{bam} \emph{to the right of the crane}
  \item \hliii{þtarhaþpa} \emph{willow} → \hliii{þtar}\hliv{-dira-}\hliii{haþpa} or \hliii{þtarhaþ}\hliv{-dira-}\hliii{pa} \emph{around the willow}
\end{itemize}
