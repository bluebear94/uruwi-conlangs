...
\begin{titlepage}
    ...
    \begin{center}
        ...
        {\large \dxvi \textcolor{Purple}{\@title} \\}
        ...
        % ----------------------------------------------------------------
        \vspace{2cm}
        {\Large\textnedham{eeŋs.-meibpelbe-kona}} \\
...
The \emph{Neðam} (\textkardinal{nez\^vam} / {\textnedham{neðam}}) script, which is used for \lname, is a boring old alphabet. The consonants and vowels are shown in tables \ref{table:ncons} and \ref{table:nvows}, respectively. Rod signals and punctuation are shown in table \ref{table:punct}.

Interestingly, most glyphs are either made entirely of straight lines or made entirely of curves. In particular, consonants that were in Lek-Tsaro are made of straight lines, although it is \hortho{s\^f} rather than \hortho{s\^h} that corresponds to Lek-Tsaro's \hortho{s\^h}. Other consonants, as well as vowels, are made of curves.
...
\begin{longtabu}{|r|>{\kardinal}Y|>{\kardinal}Y|}
    ...
    Accusative & .ime\hliii{zim} (.imezim) & .ime\hliii{rivi} (.imerivi) \\
\end{longtabu}
...
\begin{longtabu}{|r|>{\kardinal}Y|}
    ...
    \hline
    \multicolumn{2}{|l|}{\textnormal{Measure: \hortho{*mel} ``volume'' (in expressions such as \hortho{*mel-rqso} ``cupful'')}} \\
    \hline
...
\begin{table}[h]
  ...
  \begin{tabular}{l|>{\kardinal}l|>{\kardinal}l}
    ...
    Anaph. Sub. & nun\hliii{o} (nuno) & nun\hliii{ol} (nunol) \\
...
The days after those are referred to by number (one-indexed): the thirteenth day would be called \hortho{rqmq-s\^fip}.
...
The first day of \hortho{kaya-nik\^xel} coincides with the founding of the (not yet named).

\chapter{Quoting direct speech}
\label{chapter:quoting}

Direct speech is not quoted as-is, but rather it is converted into a stack code. A \emph{quotative} begins with \hortho{kpd}. Quotatives are not affected by phonorun reduction.
...
\begin{longtabu}[c]{r|l|Y}
    ...
    003 & none & Pop $n_a$ from the stack and then $n$ from the stack. Treating $n$ as a noun and $n_a$ as a noun, push the noun phrase \hortho{$n$-ma $n_a$} to the stack. \\
    ...
    \begin{itemize}
        ...
        \item Bits 5 -- 8: one of 16 intrinsic seqids. These depend on the dialect, but in the standard dialect these are \hortho{a p e t i d o g u k z t\^x v s\^h ar pr}. Unused if bit 4 is not set.
    \end{itemize}
    \\
...
\begin{itemize}
    \item A sentence with a copula and an adjective as the ``object'' cannot have any attributes attached to the copula. The workaround is to refrain from using instruction 025 and instead create a normal sentence (via 007) whose object is a dummy noun (e.~g. \hortho{maza}, \hortho{vik}) modified by the adjective.
...
\begin{figure}
    \caption{ASTs for the clauses at the deepest nesting level. NB: We translate \emph{role} as \hortho{skofl}, which has a connotation of normalcy, so we need not explicitly state that it is ``proper''. \label{fig:clauseast1}}
    \centering
    \begin{tikzpicture}
        \tikzset{every tree node/.style={align=center}}
        \tikzset{execute at begin node=\strut}
        \begin{scope}
            %\Tree [.S [.V  ] [.NP LaTeX ] [.VP [.V is ] [.NP fun ] ] ]
            \Tree [.S
                [.V compensate ]
                [.N \tsc{pr.near} ]
                [.N \tsc{pr.far} ]
                [.CONN \tsc{cond.0.}\hortho{a} ]
                ]
        \end{scope}
        \begin{scope}[yshift=-3cm]
            \Tree [.S
                [.V capture [.\tsc{past} ] [.\tsc{perfect} ] ]
                [.N chairman [.\tsc{def} ] ]
                [.N criminal [.\tsc{def} ] ]
            ]
        \end{scope}
        ...
        \begin{scope}[yshift=-6cm,xshift=2cm]
            \Tree [.S
                [.V escape [.\tsc{past} ] ]
                [.N \tsc{criminal} [.\tsc{def} ] ]
            ]
        \end{scope}
        \begin{scope}[yshift=-9cm,xshift=-5cm]
            \Tree [.S
                [.V allow [.\tsc{past} ] ]
                [.N \tsc{person} [.\tsc{def} ] ]
                [.N \tsc{pr.lc} ]
...
The seven clauses together thus form the word
\hortho{\seqsplit{%
    kldtq.enebpa.isu.asu.itiba.obite.efuzpa.ipi.q.a.atqtinabusu{n\^g}so.qtq.o{f\^x}oto{n\^g}so.qtitu.icpa.isu.afatitq.etagpa.itq.o{f\^x}oto{n\^g}so.qtatu.icpa.itq.umazaso.qfatitu.impatesu.use.oskoflse.uwa{t\^s}asa.uunun.i.etitu.icpatetq.umazaso.qfatiba.obi%
    }}.
(See \texttt{7\_1/listings/example-nestedquote.qasm} for how this was derived.)

Now we work on the middle clause, which can be unfolded as:

\begin{itemize}
    \item I have known Vladimir for 12 years.
    \item You have known Vladimir for $12 / 2 = 6$ years.
    \item Maria could have taken the opportunity to quit the Assembly without repercussions.
    \item She could have announced (...).
    \item Vladimir let either one of the above to happen. (1)
    \item She dared to listen to Vladimir.
    \item Therefore, she was ridiculed by her brothers. This was done despite (1).
    \item I found the above surprising.
    \item I do not find it surprising right now.
\end{itemize}

For the sake of brevity, we will not show the ASTs for these clauses, but see \texttt{7\_1/listings/example-quote.qasm} for how this is derived. The part outside the quotation is straightforward. Thus we have:

\begin{quote}
    \textkardinal{
        mejat .imir-ni ta, revi.el ra
        \seqsplit{te.emunpa.ipi.i.a.atq.qneto.ubane{f\^x}wu.i.a.a.asa.qkesu.atqtqvladimicvutite.emunpa.ipi.i.a.atq.qneto.ubane{f\^x}wu.i.a.a.asa{f\^x}wu.i.a.a.a.qri.qkesu.isutatitutivirlastpa.ipi.qsu.atq.upcen{k\^x}otqtafilmap{k\^x}otqtamaci.avuse.omalnaso.qtitq.erevpa.ipi.qsu.asu.utqkletq.enebpa.isu.asu.itiba.obite.efuzpa.ipi.q.a.atqtinabusu{n\^g}so.qtq.o{f\^x}oto{n\^g}so.qtitu.icpa.isu.afatitq.etagpa.itq.o{f\^x}oto{n\^g}so.qtatu.icpa.itq.umazaso.qfatitu.impatesu.use.oskoflse.uwa{t\^s}asa.uunun.i.etitu.icpatetq.umazaso.qfatiba.obititu.ictqtqvladimicvufafitq.izkotibakubitq.uvenspa.isa.o.adaz{k\^x}otqtamaci.avusu.utiba.ubitq.e{k\^x}a.pa.itq.ofqltoso.isu.utiba.ubibakubitq.emejpa.ifasu.ititq.emejpa.utq.emif{k\^x}ofisu.iti}.
    }
\end{quote}
...
\lname{} works with colour \emph{transitions}, not static colours, and uses abstract nouns to represent them. See table \ref{tab:colourtrans}.
...
\begin{longtabu}[c]{r|>{\ttfamily}Y|Y}
    ...
    010 -- 017 & literal \seqsplit{<\textit{0-7}|v\textit{1-3}|nsent|nnonsent|nmeas|nedib|nined|nabst|dfront|dback>} \textit{literal} & The literal is surrounded by double quotes and uses romanisation from hacm. Superscript letters are denoted by caret characters. \\
    020 & pronoun \seqsplit{<near|far|other|anaph\_sub|anaph\_obj|generic>} [\textbf{nd}|du] & \\
    ...
    050 & trans & \\
    051 & modverb & \\
    060 & conn \seqsplit{<ordinary|analogous|subversive|augmentative|explanatory|conditional>} <0|1> \textit{seqid} [noimm] & If \textit{seqid} is one of the intrinsic seqids, then the instruction is converted to not pop a seqid unless the noimm option is set. If a default seqid is not used and \textit{seqid} is not \texttt{-}, then a literal instruction is generated before this instruction. \\
    061 & attachconn & \\
    100 & setnname & \\
    200 & concat & \\
    700 & pushint \textit{a} & \\
...
\section{7\_1/listings/example-nestedquote.qasm}
\label{sec:listingnestedquote}

\texttt{
    \lstinputlisting[]{7_1/listings/example-nestedquote.qasm}
}

\section{7\_1/listings/example-quote.qasm}
\label{sec:listingquote}

\texttt{
    \lstinputlisting[]{7_1/listings/example-quote.qasm}
}
