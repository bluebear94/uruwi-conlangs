...
Now say that we want to translate \emph{Ryze is hiding from me in the tree with fruit.} \emph{With} would be translated as \hortho{pr} (from Lek-Tsaro \hortho{prin} \emph{hold, carry}, which also begets \hortho{rn}), but now we have nested obliques, which means we need to use \hortho{kan} as a postposition: \\
~\\
