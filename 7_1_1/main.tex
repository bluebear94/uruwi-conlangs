...
\begin{alignat*}{2}
  ...
  C_1[+na] &\rightarrow C_1[pa=x] &\quad(C_2[-na, -nareal, pa=x]) \\
...
\begin{itemize}
  \item \hortho{r} if the subject is the one participating in the main clause
  \item \hortho{rg\^j} if the direct object is participating
  \item \hortho{rkn} if the object of the preposition \hortho{kn} is participating
\end{itemize}
...
\begin{longtabu}{|r|>{\nedham}Y|}
    ...
    \hline
    \multicolumn{2}{|l|}{\textnormal{Abstract: \hortho{gxs} / \northo{gis} ``[the number] five''}} \\
    \hline
    Nominative & gis \\
