...
Vocalisation is the process of determining which phonemes to pronounce as vowels, and which as consonants. In usual speech, speakers tend to use convenient pronunciations; for instance, ingressive nasal vowels are generally avoided, as are complex consonant or vowel clusters. A speaker who is unable to pronounce [r] will usually set things up so any egressive \hortho{c}s will be pronounced as vowels instead of consonants. Nonetheless, formal vocalisation patterns are used more often in chants or songs.

\begin{itemize}
  \item Vowel-only (\hortho{sxt}): in strong forms of this vocalisation pattern, all phonemes are pronounced as vowels. Weaker versions of this pattern might pronounce ingressive nasal phonemes as consonants.
  \item CV (\hortho{nbn\^gl}): alternates between consonants and vowels, starting with a consonant. Words with odd numbers of phonemes will have the last one pronounced as a consonant. Words with one phoneme are pronounced as vowels.
  \item VC (\hortho{stbk}): alternates between vowels and consonants, starting with a vowel. Words with odd numbers of phonemes will have the last one pronounced as an extra consonant. Words with one phoneme are pronounced as vowels.
\end{itemize}

Thus, here are some vocalisations for a sentence: \\
~\\
\textkardinal{\bs{}g mgs/n\^gdjk sn\^g \bs{tgl} /kbld gl\bs{}n\^gmh sgm sd/bmy\bs{}xtn ny /kld \bs{ckm} /mxk.} \\
...
VC: \\
{}[↑o ũkɨ↓ɠejǂ ɨɠ ↑e˥kl ↓o˩pɯt ol↑ãmħ ɨkm ɨt↓ymi↑tɬʼe˥n ẽj ↓o˩lt ↑ḛkʼm ↓ũǁǂ]
...
\begin{itemize}
  \item \hli{$x$} and \hlii{$y$} are identical (and are of the same parity)
  \item they belong to sentences \hlv{$\alpha$} and \hlvi{$\beta$}, respectively (NB: it is possible that $\hlv{\alpha} = \hlvi{\beta}$)
  \item if \hli{$x$} is the \hlvii{$i$}th word of \hlv{$\alpha$}, then \hlii{$y$} is the \hlvii{$i$}th word of \hlvi{$\beta$}
  \item there are no sentences between \hlv{$\alpha$} and \hlvi{$\beta$} whose \hlvii{$i$}th word is a connector different from \hli{$x$} and \hlii{$y$} (or is of different parity)
\end{itemize}
...
\synopsis{\hlx{Variables} provide a more explicit alternative to pro-forms.}

Variables are declared to be of a type (implicitly). A type is one of the following:

\begin{itemize}
  \item a concrete
  \item an abstract
  \item a numeral (of which Boolean values are a subset)
  \item an array consisting of elements of the same type
  \item a function receiving zero or more parameters and returning a value
\end{itemize}

Concrete variables are divided into generic and specialised variables:

\begin{itemize}
  \item Generic variables can refer to different objects depending on how they are used (e.~g. \emph{\hli{X}} in \emph{if there are at least six \hli{X}es in the pile, then take one \hli{X} from your pocket and throw it in}).
  \item Specialised variables refer to the same entity every time they are used (e.~g. \emph{\hlii{Y}} in \emph{if \hlii{Y} is here, then give \hlii{Y} this paper}).
\end{itemize}

Generic variables are inflected for mutability, but they are always of the first gender.

A variable can be referred to by an arbitrary string, although in practice, generic variables have a minimum identifier length of three characters. Orthographically, they receive the sigil \hortho{\$}.

Variables can be bound by using \hlvii{lambdas}.
...
\begin{equation}
  \text{\hortho{tmlb}}\ ((v\ \text{\hortho{n}})^* v)?\ \text{\hortho{ks}}\ body\ \text{\hortho{sk}}
\end{equation}

where $v$ is an identifier. A function can subsequently be called with the following syntax:

\begin{equation}
  f\ \text{\hortho{blmt}}\ ((x\ \text{\hortho{n}})^* x)?\ \text{\hortho{gs}}
\end{equation}

Note the analogy with the lambda syntax in the argument list. $f$ need not be bound to a variable; in fact, the lack of such a requirement is the only way to bind variables!
...
\begin{equation}
  \text{\hortho{mtyw}}\ ((x\ \text{\hortho{n}})^* x)?\ \text{\hortho{wytm}}
\end{equation}

and an array is indexed as such:

\begin{equation}
  \text{\hortho{mtk}}\ A\ \text{\hortho{sg}}\ n\ \text{\hortho{gs}}
\end{equation}

As with lambdas, arrays do not need to be bound to a variable to be used. \\
~\\
\textkardinal{\hli{\bs{}gyn} \hlii{mc/klbm} \hliii{lk} \hliv{bl\bs{}yxn\^gw} \hlv{/nkhgm}} \\
\textkardinal{\hli{gyn} \hlii{mcs->h:lbm} \hliii{lk} \hliv{bls->n\^g:xn\^gw} \hlv{>nkhgm}} \\
\hli{\tsc{qp.num}} \hlii{\tsc{pr0.listener}-0-$\Omega$} \hliii{want} \hliv{piece-1-$\Sigma$} \hlv{\tsc{desc}-sugar} \\
\textkardinal{\hli{mtk} \hlii{mtyw} \hliii{jn\^g} \hliv{n} \hlv{jy} \hlvi{wytm} \hlvii{sg} \hlviii{yng} \hlix{gs}} \\
\hli{\tsc{index}(} \hlii{\tsc{array}(} \hliii{two} \hliv{,} \hlv{three} \hlvi{)\tsc{array}} \hlvii{\tsc{index,}} \hlviii{\tsc{qw.num}} \hlix{)\tsc{index}} \\
\emph{What is the number $i$ for which you want $[2, 3][i]$ pieces of sugar?} \\
or more idiomatically: \emph{Do you want two or three pieces of sugar?}
...
(Another way to describe this division is that, in terms of more conventional terminology, \hli{coördinating} compounds usually cover copulative and appositional compounds, and \hlii{subordinating} compounds usually cover endocentric and exocentric compounds.)
