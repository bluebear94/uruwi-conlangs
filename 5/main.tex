...
\usepackage[dhr,shortsuper,hacm]{common/uruwi}
...
\begin{itemize}
    ...
    \item Due to uncanny font magic, \emph{Nasél Tēkel Piva,} the script of Ḋraħýl Rase
        is also covered.
\end{itemize}
...
\begin{table}[h]
    \caption{The consonants of Ḋraħýl Rase, with their romanisations.}
    \centering
...
\begin{table}[h]
\centering
    \caption{The vowels of Ḋraħýl Rase, with their romanisations.}
    \begin{tabular}{lll}
...
A word has one stresed syllable. The natural location of the stressed syllable is determined by the following rules:

\begin{itemize}
    \item If there is a long vowel or a diphthong in the last three syllables, then the stress falls on one of them, in the order 2nd-to-last → 3rd-to-last → last.
    \item Otherwise, the stress falls on the second-to-last syllable.
\end{itemize}

Stress will be indicated in this grammar. If it falls on its natural location, then it is not marked. Otherwise, long syllables that are forced unstressed will be written with macra, and short syllables that are forced stressed will be written with acutes.
...
Hyphens may separate parts of words. In that case, only the last part will be counted toward translating between the romanisation and the actual location of stress.
...
\begin{table}[ht]
    \caption{Symbols used to show stress shifting. \label{table:shift}}
    \centering
    \begin{tabu} to \linewidth {lX}
        \hline
        Symbol & Meaning \\
        \hline
        \sshift & Shift stress one syllable forward \\
        \sshiftp & Shift stress to second-to-last syllable \\
        \sshiftu & Shift stress to last syllable \\
        \sstay & Keep stress on same syllable \\
        \hline
    \end{tabu}
\end{table}
...
\section{Nasél Tēkel Piva}

Ḋraħýl Rase is written in \emph{Nasél Tēkel Piva} (\emph{lit.} New Sun Glyphs), a native script that uses dedicated glyphs for consonants and long vowels, plus diacritics for short vowels. It does not mark stress.
...
As seen in Table \ref{table:scriptex}, names receive an overline.

\section{Hacm}

Ḋraħýl Rase can also be written in hacm. The orthography does not mark stress.

\begin{longtabu}{>{\kardinal}ll|>{\kardinal}ll|>{\kardinal}ll|>{\kardinal}ll}
    \caption{Consonant, semivowels and vowels in hacm. Long vowels are written with doubled letters.} \\
    \textnormal{Hacm} & Rom &
    \textnormal{Hacm} & Rom &
    \textnormal{Hacm} & Rom &
    \textnormal{Hacm} & Rom \\
...
The convention on notating numerals is divided. Some factions prefer to use the letters \hortho{t k} for 10 and 11, others prefer mirrored versions of \hortho{5 7} (\hortho{\reflectbox{5} \reflectbox{7}}), and yet others prefer to use the NTP numerals.

Punctuation, on the other hand, is hardly controversial -- the hacm equivalents of the appropriate punctuation marks are used.

\begin{table}
    \caption{An example with names. The colours are solely for emphasis.}
    \label{table:scriptex}
    \centering
    \begin{tabu} to \linewidth {X}
        ...
        \hline
        \textdhr{
            malnelkAjkane xAlemulama Danomulama luneksi \hli{\textoverline{alis}} ruselmara. sel ka mon Sunama Danos lumekAl sydasAW panetaki sydasAWma rihu ka tUrI \tl etera. [rihu ka tUrI \tl etekAl sydasAWma \tl lumetSalu] tes vanretara. \strut
        }
        \\
        \hline
        Hacm \\
        \hline
        \textkardinal{
            malnelkaykane k\^xaale-mulama d\^aano-mulama luneksi \hli{alis} ruselmara. sel ka mon xunama d\^aanos lumekaal su\^idasaw\^i panetaki su\^idasaw\^ima rihu ka tuurii keel etera. "rihu ka tuurii keel etekaal su\^idasaw\^ima keel lumetxalu?" tes vanretara. \strut
        }
        \\
        \hline
    \end{tabu}
...
Descriptors, which modify nouns, consist of genitives, numerals, relative clauses, locatives and directionals, although instances of the latter two categories might be modifying a verb instead. They come \emph{before} the noun they modify.

Let us define the following properties for descriptors:

\begin{itemize}
    \item A \emph{restrictive} descriptor narrows the set of objects that the antecedent refers to. A \emph{nonrestrictive} descriptor merely provides supplementary information. This distinction is shown in relative clauses using \ortho{\sshiftu \=/kâl} and \ortho{\sshiftu \=/kâṡ}, respectively, but for genitives, it must be inferred via word order.
    \item An \emph{objective} descriptor names a property that can be agreed on by any two observers. A \emph{subjective} descriptor names a property that two observers might disagree on. \\
    Ex: colour is objective, but beauty is subjective
    \item An \emph{engineered} descriptor describes a property that the antecedent is engineered to bear. A \emph{natural} descriptor is the opposite. \\
    Ex: size of a cultivated crop is engineered, but size of a wild plant is natural
\end{itemize}

Note that some of the properties above are dependent on the antecedent of a descriptor as well as the descriptor itself.

The descriptors of a noun appear in the order described by table \ref{table:descriptor_order}.

\begin{table}[h]
    \caption{Order of descriptors, with the earliest at the top.}
    \label{table:descriptor_order}
    \centering
    \begin{tabular}{r|l}
        \# & Category \\
        \hline
        1 & Relative clauses \\
        2 & Nonrestrictive genitives \\
        3 & Restrictive subjective genitives \\
        4 & Locatives and directionals \\
        5 & Numerals \\
        6 & Restrictive objective engineered genitives \\
        7 & Restrictive objective natural genitives \\
        8 & Genitives that act as determiners \\
    \end{tabular}
\end{table}

\section{Adverbials}

Adverbials include true adverbs, adverbial forms of nouns, locatives and directionals. They can occur anywhere before the verb they modify, although true adverbs are more likely than other adverbials to appear at the very beginning of a clause.
...
\begin{itemize}
    ...
    \item otherwise:
    \begin{itemize}
        \item appending \ortho{\sshift -l} after a vowel if the stress is not on the final syllable
        \item appending \ortho{\sshift -li} after a consonant, or if the stress is on the final syllable
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item otherwise:
    \begin{itemize}
        \item appending \ortho{\sshift -r} after a vowel if the stress is not on the final syllable
        \item appending \ortho{\sshift -ri} after a consonant, or if the stress is on the final syllable
    \end{itemize}
\end{itemize}
...
There are suppletive forms of genitive pronouns (table \ref{table:genpronouns}), but they are somewhat archaic. It is more common to derive them regularly from their absolutive forms.

\begin{table}[ht]
    \caption{Genitive pronouns. \label{table:genpronouns}} 
    \centering
    \begin{tabular}{|l|l|l|l|}
        \hline
        Class \bs{} Person & 1st & 2nd & 3rd \\
        \hline
        Divine & erul & nîl & \invalid \\
        Elite & nabál & rêl & ħranúl \\
        Non-elite & êl & sûl & anél \\
        Animate & \invalid & \invalid & nêl \\
        Inanimate & \invalid & \invalid & val \\
        \hline
    \end{tabular}
\end{table}
...
The infinitive form of a verb ends in \ortho{\=/ek}. Additionally, the stress does not fall on the last syllable.
...
\begin{longtabu} to \linewidth {|l|l|Y|}
    ...
    Repetitive & -ne & Indicates that an action happened in the past and is happening again. \\
...
If the stress of the infinitive falls on the natural location, then it will for any conjugated form.
...
An independent clause may have its tense marker (and possibly aspect markers) omitted if a clause later down the chain holds such markers: \\
~\\
\hli{Ṡaga} \hlii{nevur-ħajmá} \hliii{tanake,} \hliv{sevy} \hlv{paneto,} \hlvi{vozas} \hlvii{haldet} \hlviii{syltesra.} \\
\hli{half} \hlii{two\_hours-through-\tsc{loc}} \hliii{walk-1-\tsc{prog},} \hliv{home} \hlv{see-3-1,} \hlvi{mud-\tsc{erg}} \hlvii{leg-\tsc{du}} \hlviii{cover-3\tsc{du}-\tsc{past}} \\
\hliii{I walked} \hlii{for an hour,} \hlv{looked at} \hliv{my house,} \hlvi{mud} \hlviii{covering} \hlvii{my legs.} \\
~

Note that \ortho{\hliii{tanake}} and \ortho{\hlv{paneto}} receive no tense marker because it is provided by \ortho{\hlviii{syltesra}}.

Otherwise, tense is absolute among independent clauses: \\
~\\
\hli{Sepletys} \hlii{daj-manema} \hliii{suṡara,} \hliv{nepletys} \hlv{lykoj-manema} \hlvi{suṡaṫa.} \\
\hli{\tsc{fem}-parent-\tsc{erg}} \hlii{previous-day-\tsc{loc}} \hliii{visit-1-\tsc{past},} \hliv{\tsc{masc}-parent-\tsc{erg}} \hlv{next-day-\tsc{loc}} \hlvi{visit-1-\tsc{fut}} \\
\hli{My mother} \hliii{visited me} \hlii{yesterday,} \hliv{and my father} \hlvi{will visit me} \hlv{tomorrow.} \\
~

If it wasn't obvious from the other words, the word \ortho{\hlvi{suṡaṫa}} refers to the future in the absolute sense, rather than relative to when the event described by the first clause happened.

In a single independent clause with one or more dependent clauses, the clause that occurs first is absolute, and all other clauses are relative to their predecessors: \\
~\\
\hli{Daj-manema} \hlii{penarakâl} \hliii{rekko} \hliv{hẏo} \hlv{kotanelṫa.} \\
\hli{previous-day-\tsc{loc}} \hlii{see\bs{}\tsc{pl}-1-\tsc{past}-\tsc{rel}} \hliii{young\_person} \hliv{now} \hlv{arrive-3\tsc{anm}-\tsc{fut}} \\
\hliii{The young man} \hlii{whom we saw} \hli{yesterday} \hlv{has arrived} \hliv{now}. \\
~

Here, \ortho{\hlv{kotanelṫa}} indicates that the arrival of the young man happened after the speakers saw him (\ortho{\hlii{penarakâl}}).

\chapter{Numerals}

\section{Cardinal numerals}

Ḋraħýl Rase uses a base-12 numbering system. The cardinal numerals from 0 -- 12 are given below:

\begin{longtable}[c]{|l|l|}
    \caption{The cardinal numerals from 0 -- 12.} \\
...
Words for integers in the form $x \cdot 12$ for $2 \le x < 12$ are expressed as \ortho{$x$-vôn} -- e.~g. \ortho{monvôn} = 24; \ortho{rejvôn} = 108.

Words for integers in the form $x \cdot 12 + y$ for $2 \le x < 12$ and $1 \le y < 12$ are expressed as \ortho{$x$-vôn-$y$}, but with a few exceptions:
...
Words for integers less than $12^6$ are expressed in the form \[a \cdot 12^5 + b \cdot 12^4 + c \cdot 12^3 + d \cdot 12^2 + (x \cdot 12 + y)\]
...
Words for integers that are $12^6$ or greater are split into groups of six digits and use the following words for powers of $12^6$:
...
The ordinal numerals for \emph{1st} and \emph{2nd} are the suppletive forms \ortho{troma} and \ortho{iramu}, respectively. Most ordinals after \emph{2nd} are expressed regularly with the suffix \ortho{\=/ru}.
...
\begin{itemize}
    ...
    \item The stress is shifted to the second-to-last syllable.
\end{itemize}
...
~\\
\hliv{Ṅarkus} \hlii{lavatu} \hli{neke,} \hliii{ej} \hlv{luṡuhatakjusit.} \\
\hliv{Ṅarku-\tsc{erg}} \hlii{bread-\tsc{additional}} \hli{dislike-3,} \hliii{\tsc{pr.1}} \hlv{annoy-1-3-\tsc{continuative}-\tsc{habitual}} \\
\emph{\hliv{Ṅarku} \hli{dislikes} \hlii{even bread,} and \hlv{she keeps annoying} \hliii{me}}. \\
\emph{(Here, \tsc{abs} is occupied, so \ortho{Ṅarku} takes \tsc{erg} in the second clause.)}

\section{\ortho{ka} and \ortho{gy}}

\ortho{ka} (inclusive \emph{or}) and \ortho{gy} (exclusive \emph{or}) are explicit conjunctions between what they join, and they follow the same rules as juxtaposition.
...
It is more common to use the adverbial case: \ortho{munuma} \emph{slowness} $\rightarrow$ \ortho{munumár} \emph{slowly}.
...
This is a simple suffix \ortho{\sshift-nek} and produces an intransitive verb. If the stress somehow falls on the last syllable with the shift, it instead falls on the second-to-last: \ortho{ħajnek} instead of \wrong{ħajnék}.
...
Abbreviations of phrases with multiple words take the body of the inital syllable of each word, shortening long vowels and removing the glides from diphthongs: \ortho{Nasél Tēkel Piva} shortens to \ortho{Natepi}, for instance.

\section{Personal names}

Most citizens of Ḋraħyn-Nŷr have a familiar given-surname combination, in that order.

\begin{itemize}
    \item The given name is usually a noun standing for a concrete object, usually of a natural object or phenomenon.
    \item The surname is usually a place name or an adjectival noun. Inherited from the mother if born during the day, and from the father if born at night.
\end{itemize}

All parts of a name are inflected for case individually.

\section{Colour}

The traditional colour terms are listed in table \ref{table:colours}.

\begin{table}[h]
    \caption{Traditional colour terms in Ḋraħýl Rase.}
    \label{table:colours}
    \centering
    \begin{tabular}{l|l}
        Term & Translation \\
...
Over time, the words for ``red'' and ``green'' were reänalysed as the compounds ``magenta-yellow'' and ``cyan-yellow'', yielding \ortho{uksu} \emph{magenta} and \ortho{reh} \emph{cyan}. This also yielded one of the words for colour: \ortho{dan}. These terms, along with the bound root \ortho{en}, are combined to create terms for high-chroma colours such as those from table \ref{table:colours2}.

\begin{table}[h]
    \caption{Some terms derived from \ortho{uksu} and \ortho{reh}.}
    \label{table:colours2}
    \centering
    \begin{tabu} to \linewidth {l|lY}
        Term & Translation & Derivation \\
        \hline
        uksu & magenta & basickish \\
        reh & cyan & basickish \\
        \hline
        leksubên & scarlet & len uksu + bê en \\
        enuksun & orange & en + uksu + en \\
        enbênreh & chartreuse & en + bê en + reh \\
        enmorehen & green-yellow & en + mon rehen \\
        morehselen & aquamarine & mon reh + sel en \\
        resuhor & sky blue & reh + suhor \\
        rehuksureh & purple & reh + uksu + reh \\
        ṫaksumoreh & violet & ṫak uksu + mon reh \\
        uksurehuksu & cerise & uksu + reh + uksu \\
    \end{tabu}
\end{table}

Quite unsurprisingly, terms for tints are formed with the prefix \ortho{revet-} and those for shades are formed with the prefix \ortho{moru-}. Before a vowel or with the colour terms in table \ref{table:colours}, \ortho{mor-} is used for shades instead. Terms for tones are prefixed with \ortho{ḋalfo-} or \ortho{rem-}.

There are also a few colour terms that do not fall in the above categories (table \ref{table:colours3}).

\begin{table}[h]
    \caption{Miscellaneous colour terms.}
    \label{table:colours3}
    \centering
    \begin{tabu} to \linewidth {l|lY}
        Term & Translation & Derivation \\
        \hline
        enḋalfo & warm grey & en + ḋalfo \\
        neṡkemoru & brown & neṡke \emph{(dirt)} + moru \\
    \end{tabu}
\end{table}

\section{Kinship terms}

Kinship terms in Ḋraħýl Rase are mostly not distinguished by gender, although there are separate terms for parallel and cross cousins and their parents.
...
\begin{longtable}[c]{l|l}
    ...
    plety & parent \\
    sulmo & grandparent \\
    ḣelfo & great-grandparent \\
    ruma & child \\
    véruma & grandchild \\
    ḋano & older sibling \\
    silu & younger sibling \\
    bardu & father's brother or mother's sister \\
    lisku & father's sister or mother's brother \\
    gżalim & parallel first cousin \\
    kiḋuṅ & cross first cousin \\
    nilbardu & grandfather's brother or grandmother's sister \\
    nilisku & grandfather's sister or grandmother's brother \\
    nilgżalim & parent of parallel second cousin \\
    nilkiḋuṅ & parent of cross second cousin \\
    gitak & parallel second cousin \\
    alfur & cross second cousin \\
    ḣespu & child of any cousin \\
\end{longtable}
...
\hli{lakanma} \hlii{kêl} \hliii{zanekekâl} \hliv{meṡa,} \hlv{nurema} \hlvi{klūdli} \hlvii{fṡube.} \\
...
\hli{Idisa} / \hlii{nēmá} \hliii{bê} \hliv{ryma} \hlv{ħeli-koloma} \hlvi{huvrelke.} \\
...
\hli{Nefiḋo} \hlii{lenekâl} \hliii{kolo-mojmé,} \hliv{vaneḋo} \hlv{lenekâl} \hlvi{kolome,} \\
...
A translation of an Arka poem\footnote{\url{http://conlinguistics.org/arka/images/semaim.png}}. \\
~\\
\textdhr{kitOku malneltora} \\
\hli{Kitôku} \hlii{malneltora} \\
\hli{\tsc{dim}-cat} \hlii{pick\_up-3\tsc{anm}-1-\tsc{past}} \\
\hlii{I picked up} \hli{a little cat} \\
~\\
\textdhr{tEdekli mevutanul mane} \\
\hli{Tēdekli} \hlii{mevutanúl} \hliii{mane} \\
\hli{Têdek-\tsc{gen}} \hlii{rain-\tsc{bounty}-\tsc{gen}} \hliii{day} \\
\hlii{A rainy} \hliii{day} \hli{in June} \\
~\\
\textdhr{koval pSemadural tjAlorli setlamaGama} \\
\hli{Kovál} \hlii{pṡema-durál} \hliii{tjalorlí} \hliv{setla-maġama} \\
\hli{young-\tsc{gen}} \hlii{sprout-glue-\tsc{gen}} \hliii{hydrangea-\tsc{gen}} \hliv{leaf\bs{\tsc{pl}}-below-\tsc{loc}} \\
\hliv{Under the leaves} \hliii{of the hydrangea} \hlii{with} \hli{young} \hlii{sprouts,} \\
~\\
\textdhr{mevun nepSelkerakAS kitOku malnelto} \\
\hli{Mevun} \hlii{nepṡelkerakâṡ} \hliii{kitôku} \hliv{malnelto} \\
\hli{rain-\tsc{acc}} \hlii{take\_shelter-3\tsc{anm}-\tsc{prog}-\tsc{past}-\tsc{rel.nonrestrictive}} \hliii{\tsc{dim}-cat} \hliv{pick\_up-3\tsc{anm}-1} \\
\hliv{I picked up} \hliii{the little cat,} \hlii{hiding from} \hli{the rain} \\
\emph{(Note the sequence of tenses.)} \\
~\\
\textdhr{revetli tOku rAjnetora} \\
\hli{Revetlí} \hlii{tôku} \hliii{rajnetora} \\
\hli{white-\tsc{gen}} \hlii{cat} \hliii{sleep-3-1-\tsc{past}} \\
\hliii{I put} \hli{the white} \hlii{cat} \hliii{to rest} \\
\emph{(The patient is considered inanimate here for somewhat obvious reasons.)} \\
~\\
\textdhr{tEdekli mevutanul mane} \\
\hli{Tēdekli} \hlii{mevutanúl} \hliii{mane} \\
\hli{Têdek-\tsc{gen}} \hlii{rain-\tsc{bounty}-\tsc{gen}} \hliii{day} \\
\hlii{A rainy} \hliii{day} \hli{in June} \\
~\\
\textdhr{dunEwma tjAlorli setlamaGame} \\
\hli{Dunewmá} \hlii{tjalorlí} \hliii{setla-maġame} \\
\hli{encounter-\tsc{v>n}-\tsc{loc}} \hlii{hydrangea-\tsc{gen}} \hliii{leaf\bs{\tsc{pl}}-below-\tsc{dir}} \\
\hliii{Under the leaves} \hlii{of the hydrangea} \hli{where we met,} \\
~\\
\textdhr{kolonelrakAS tOku rAjnetoTa} \\
\hli{Kolonelrakâṡ} \hlii{tôku} \hliii{rajnetoṫa.} \\
\hli{large-become-3\tsc{anm}-\tsc{past}-\tsc{rel}} \hlii{cat} \hliii{sleep-3-1-\tsc{fut}} \\
\hliii{I put to rest} \hli{the grown} \hlii{cat} \\
\emph{(Note the sequence of tenses again, as well as the use of both the animate and inanimate conjugations when referring to the cat.)} \\
~\\
\textdhr{misoto mik neSke bZyretora} \\
\hli{Misoto} \hlii{mik} \hliii{neṡke} \hliv{bżyretora} \\
\hli{thank-0-1} \hlii{\tsc{simul}} \hliii{dirt} \hliv{pile\_up-3-1-\tsc{past}} \\
\hli{I gave thanks} \hlii{while} \hliv{laying down} \hliii{the dirt} \\
\emph{(The sequence of tenses between independent clauses is different than when a dependent clause is involved.)} \\
~\\
\textdhr{misoto mik neSke bZyretora} \\
\hli{Misoto} \hlii{mik} \hliii{neṡke} \hliv{bżyretora} \\
\hli{thank-0-1} \hlii{\tsc{simul}} \hliii{dirt} \hliv{pile\_up-3-1-\tsc{past}} \\
